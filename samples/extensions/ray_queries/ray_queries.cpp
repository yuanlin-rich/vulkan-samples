...
void RayQueries::load_scene()
{
	...
	// 遍历scene中的所有mesh，将mesh的vertex和index指定到一个数组中
	load_node(scene->get_root_node());
}

void RayQueries::create_descriptor_pool()
{
	// 描述符池，包含一个加速结构和一个uniform buffer
...
{
	// 创建图形管线，光线查询可以在任意的shader中进行（vertex shader，fragment shader，compute shader）
...
{
	// uniform数据，包含相机位置，相机矩阵，投影矩阵，光源位置等

...
void HelloTriangle::init_vertex_buffer()
{
	...
	VmaAllocationCreateInfo buffer_alloc_ci{
		...
		// 让 VMA 根据你指定的 “使用场景”，自动选择最合适的内存类型（Memory Type）
	    .usage         = VMA_MEMORY_USAGE_AUTO, 
		// 写入数据之后不需要flash到GPU可见内存
...
void HelloTriangle::init_render_pass()
{
	...
	VkSubpassDependency dependency{
		...
		// 依赖的目标是子通道0
	    .dstSubpass   = 0,
		// 本subpass开始前，必须等待颜色输出完成
	    .srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
		// 颜色输出开始前，必须等待本subpass完成
	    .dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT};

	// Since we changed the image layout, we need to make the memory visible to
	// color attachment to modify.
	// 来源方执行了哪些类型的内存访问。同步需要确保这些访问先完成
	dependency.srcAccessMask = 0;

	// 目标方将要执行哪些类型的内存访问，同步会确保这些访问能安全开始
